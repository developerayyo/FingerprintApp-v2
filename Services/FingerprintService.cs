...
namespace ERPNextFingerprintApp.Services
{
    public class FingerprintService : IDisposable
    {
        ...
        private void OnFingerprintCaptured(object? sender, byte[] fingerprintData)
        {
            try
            {
                ...
                if (fingerprintData != null && fingerprintData.Length > 0)
                {
                    ...
                    var result = FingerprintCaptureResult.Success(template);
        ...
        public async Task<FingerprintVerificationResult> VerifyAsync(IEnumerable<Employee> employees)
        {
            ...
            const int minQualityThreshold = 60; // Minimum acceptable quality score
        ...
        /// <summary>
        /// Extract quality score from capture result
        /// </summary>
        private int ExtractQualityScore(FingerprintCaptureResult captureResult)
        {
            // Try to extract quality information from the capture result
            // This would typically come from the enhanced capture process
            
            // For now, we'll use a basic quality estimation based on template length and success
            ...
            // Additional quality factors could be added here based on SDK feedback
        ...
        /// <summary>
        /// Provide quality-based feedback to users
        /// </summary>
        private string GetQualityFeedback(int qualityScore)
        {
        ...
        private string CreateEnrollmentTemplate(List<string> templates)
        {
            try
            {
                // For now, use the first template as the enrollment template
                // In a real implementation, you would use DigitalPersona's template fusion
                // Example: DPUruNet.Enrollment.CreateEnrollmentFmd(fmdArray)
                
                if (templates.Count > 0)
                {
                    Log.Information("Creating enrollment template from {Count} captured templates", templates.Count);
                    // Return the best quality template or fused template
    ...
    public class FingerprintCaptureResult
    {
        ...
        public static FingerprintCaptureResult Success(string template)
        {
